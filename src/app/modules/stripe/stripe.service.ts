...
const stripLinkAccount = async (userId: string) => {
  ...
  try {
    ...
    const accountLink = await stripe.accountLinks.create({
      account: account.id,
      return_url: `${config?.client_Url}/seller/confirmation?userId=${user._id}&stripeAccountId=${account.id}`,
      refresh_url: `${config?.server_url}/stripe/refresh/${account.id}?userId=${user?._id}`,
...
const refresh = async (paymentId: string, query: Record<string, any>) => {
  ...
  try {
    const accountLink = await stripe.accountLinks.create({
      account: paymentId,
      return_url: `${config?.client_Url}/seller/confirmation?userId=${user._id}&stripeAccountId=${paymentId}`,
      refresh_url: `${config?.server_url}/stripe/refresh/${paymentId}?userId=${user?._id}`,
      type: 'account_onboarding',
    });
...
const returnUrl = async (payload: {
  ...
  try {
    ...
    return { url: 'https://dashboard.jusbuy.net/login' };
  } catch (error: any) {
    throw new AppError(httpStatus.BAD_REQUEST, error.message);
  }
};
